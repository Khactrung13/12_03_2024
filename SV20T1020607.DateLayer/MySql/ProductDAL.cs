        ...
        public long AddPhoto(ProductPhoto data)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"INSERT INTO ProductPhotos (ProductID, Photo, Description, DisplayOrder,IsHidden)
                            SELECT @ProductID, @Photo, @Description, @DisplayOrder, @IsHidden
                            FROM dual
                            WHERE NOT EXISTS (
                                SELECT * FROM ProductPhotos WHERE DisplayOrder = @DisplayOrder
                            );
                            SELECT IF(ROW_COUNT() > 0, LAST_INSERT_ID(), 0) AS InsertedID;";
                var parameters = new
                {
        ...
        public bool UpdatePhoto(ProductPhoto data)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"UPDATE ProductPhotos 
                    ...
                    AND NOT EXISTS(SELECT * FROM ProductPhotos WHERE PhotoID <> @PhotoID AND DisplayOrder = @DisplayOrder)";
                var parameters = new
                {
